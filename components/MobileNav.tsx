    ...
    return (
        <div className="md:hidden fixed bottom-1 left-0 right-0 z-50 flex w-full justify-center mb-4 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <nav className="pointer-events-auto relative flex flex-row justify-center items-center w-full">
                ...
                <ul className="flex rounded-full px-3 text-sm font-semibold text-zinc-800 shadow-lg ring-1 shadow-zinc-800/5 ring-zinc-900/5 backdrop-blur-sm bg-white/60 dark:bg-zinc-800/60 dark:text-zinc-200 dark:ring-white/10">
                    {links.map((link) => {
                        const isActive =
                            link.href === "/"
                                ? pathname === "/"
